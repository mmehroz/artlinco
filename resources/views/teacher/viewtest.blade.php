...
<form action="{{ URL::to('/submittestadmin')}}" method="POST" class="form-horizontal" enctype="multipart/form-data" style="width: 100%;">
  ...
  <input type="hidden" name="hdntestid" value="{{$data->test_id}}">
...
<div class="row mt-3">
  ...
  <div class="field_wrapper text-left mt-5">
      <div class="inner_field aop">
          ...
          <h5 class="questioninputheading">Question</h5>
          ...
          <?php
            ...
            ->get();
          ?>
                <h5 class="answerinputheading">Answer</h5>
          @foreach($answer as $answers)
          <div class="d-flex">
                                  <input type="radio" name="rd{{$questions->testquestions_id}}" value="{{$answers->testanswers_id}}">
          <input type="text" class="answerdivs" placeholder="Write Answer" name="ans{{$answers->testanswers_id}}" value="{{$answers->testanswers_answer}}" required/>
          </div>
          @endforeach
          @endforeach
      </div>
